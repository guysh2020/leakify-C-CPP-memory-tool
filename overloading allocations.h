#include<iostream>
#include <cstdio>
#include <cstdlib>


FILE* fp = fopen("allocations.txt","w");

void* my_malloc(size_t size, const char *file, int line, const char *func)
{
    void *p = malloc(size);
    fprintf (fp,"Allocated = %s, %i, %s, #%p[%li]\n", file, line, func, p, size);
    return p;
}
void* my_calloc(size_t count, size_t size, const char *file, int line, const char *func)
{
    void *p = calloc(count,size);
    fprintf (fp,"Allocated = %s, %i, %s, #%p[%li]\n", file, line, func, p, size);
    return p;
}

void my_free(void *ptr, const char *file, int line, const char *func)
{
    fprintf (fp,"freeing = %s, %i, %s, #%p\n", file, line, func, ptr);
    free(ptr);
}

void* operator new(std::size_t sz,const char* file, unsigned int line,const char *func) 
{
    void *ptr = std::malloc(sz);
    fprintf(fp,"Allocated = %s, %i, %s, #%p[%zu]\n",file,line,func,ptr,sz); 
    return ptr;
}

void* operator new[](std::size_t sz,const char* file, unsigned int line,const char *func) 
{
    void *ptr = std::malloc(sz);
    fprintf(fp,"Allocated = %s, %i, %s, #%p[%zu]\n",file,line,func,ptr,sz); 
    return ptr;
}

void operator delete(void* ptr, std::size_t sz) 
{
    fprintf(fp,"freeing = #%p[%zu]\n",ptr,sz); 
    free(ptr);
}

void operator delete[](void* ptr, std::size_t sz) 
{
    fprintf(fp,"freeing = #%p[%zu]\n",ptr,sz);
    free(ptr);
}

#define malloc(X) my_malloc( X, __FILE__, __LINE__, __FUNCTION__)
#define calloc(X,Y) my_calloc( X,Y, __FILE__, __LINE__, __FUNCTION__)
#define free(X) my_free( X, __FILE__, __LINE__, __FUNCTION__)
#define new new(__FILE__, __LINE__,__FUNCTION__)
//#define delete(X) delete(X,__FILE__, __LINE__,__FUNCTION__) //not working
